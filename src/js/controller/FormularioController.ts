       ...
       instanceForm() {
        ...
        getFormElements(newformid){
            ...
            axios.get('https://dev-trazalog.com.ar:8246/arbolado/api/ds/v1.0.0/formulario/'+newformid,  {
                ...
                .then(response => {
                    let itemsCount = response.data.formulario.items.length;
                    ...
                    for (let i = 0; i < itemsCount; i++){
                        ...
                        this.formElements.push(response.data.formulario.items[i].tipo);
                    }
                    ...
                    if (this.taza != "Taza con árbol") {
                        ...
                        for (let i = 0; i < itemsCount; i++){
                            let id_serial_Aux = response.data.formulario.items[i].id_serial;
                            let tipo_Aux = response.data.formulario.items[i].tipo;
                            let deshabilitado_Aux = response.data.formulario.items[i].deshabilitado;
                            let label_Aux = response.data.formulario.items[i].label;
                            let requerido_Aux = response.data.formulario.items[i].requerido;
                            let nombre_Aux = response.data.formulario.items[i].nombre;
                            let cond_habilitado_Aux = response.data.formulario.items[i].cond_habilitado;
                            let cond_mostrar_Aux = response.data.formulario.items[i].cond_mostrar;
                            let name_Aux = response.data.formulario.items[i].name;
                            let orden_Aux = response.data.formulario.items[i].orden;
                            let id_Aux = response.data.formulario.items[i].id;
                            let valores_Aux = response.data.formulario.items[i].valores.valor;
                            let mostrar_Aux = response.data.formulario.items[i].mostrar;
                            let actual_value = response.data.formulario.items[i].valor; /*This is the actualValue for this item*/
