...
#print_strict_params on
 ...
 v_depa_id arb_departamentos.depa_id%type; BEGIN
 BEGIN
/** busco el dire_id **/
...
/** TODO: JOINEAR CON DIRECCIONES TAMBIEN*/    RAISE INFO 'v_dire_id % - p_calle % - p_altura % -> En bloque 1', v_dire_id,p_calle,p_altura;    exception
    when NO_DATA_FOUND then        begin
        ...
        and ag.arge_id = ma.arge_id;        /** Intento conseguir la calle si existe, sino la inserto **/
        select ca.call_id into strict v_call_id
        from arb_calles ca
        where ca.nombre = p_calle
        and ca.depa_id = v_depa_id;exception
...
/**TODO: ACA AGREGAR E BARRIO  */
    ...
    values(v_call_id, p_altura, p_manz_id, p_barrio);    RAISE INFO 'p_calle % - p_altura % - p_manz_id % -> En bloque 2', p_calle,p_altura,p_manz_id;/** guardo id de direccion en v_dire_id **/
    select dire_id into strict v_dire_id
    from arb_direcciones
    inner join arb_manzanas on arb_manzanas.manz_id = arb_direcciones.manz_id
    where arb_direcciones.call_id = v_call_id
    and arb_direcciones.altura = p_altura
    and arb_direcciones.manz_id = p_manz_id;    RAISE INFO 'v_dire_id % -> En bloque 3', v_dire_id;
END; /** finalmente inserto el nuevo arbol **/
    insert into arb_arboles (nombre, info_id, cens_id, lat, long, tipo, imagen, dire_id, taza )
    values (p_nombre, case when p_info_id=0 then null else p_info_id end, p_cens_id, p_lat, p_long, p_tipo, p_imagen, v_dire_id, p_taza);return 'OK';
   /** vuelve las transacciones efectuadas si alguna falla **/
 exception
 when others then
--   rollback;
return 'ERROR: '||sqlerrm||' '|| SQLSTATE;
 --raise notice '% %', SQLERRM, SQLSTATE;END;
 $function$
;